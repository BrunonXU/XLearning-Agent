...
> 说明：本节用于记录最近一次代码回归，不删除历史 TODO 内容，仅追加状态。

- ✅ 已修复：首次发送后输入框报错（`st.session_state.chat_input_val` 赋值时机问题）
- ✅ 已修复：普通问答被强制插入“先生成计划”模板的问题（协调模式问答优先）
- ✅ 已修复：意图路由误判到 Quiz/Report 的保护逻辑（显式关键词门控 + fallback）
- ✅ 已修复：跨会话状态污染（新会话自动重置 orchestrator 状态与缓存）
- ✅ 已增强：Trace 中 `IntentDetection` 增加 source/state 细节，便于和 LangSmith 对齐

### 下一步（按优先级）

- [ ] 跑一次端到端验收：GitHub URL → 规划/学习 → 测验 → 报告
- [ ] 验证 LangSmith 全链路追踪是否完整（Intent / Planner / Tutor / Validator）
- [ ] 完成 3 分钟演示脚本与计时演练
...
### 侧边栏（任务 5）
1. 查看左侧 Sidebar 最底部
2. **验收**：✓ 显示「v0.3.0 | LangSmith ✅」或「LangSmith ❌」（根据 API Key 是否配置）
3. 展开「🛠️ 开发者选项」
...
### BF-1. LLM API 超时 ✅
- **文件**: `src/providers/tongyi.py`
- **修复**: 添加 `request_timeout=60` 参数，防止长时间无响应挂起

### BF-2. RAG 跨 Session 污染 ✅
- **文件**: `src/agents/orchestrator.py`
- **修复**: `reset()` 时保留 `domain` 和 `rag_engine`，避免新 session 绑定时销毁 RAG 上下文

### BF-3. doc_meta 系统 ✅
- **文件**: `src/agents/tutor.py`, `src/agents/orchestrator.py`
- **修复**: 新增 `doc_meta` 机制，让 Tutor 知道用户上传了 PDF，prompt 中提示 LLM 参考材料来自上传文档

### BF-4. 意图识别增强 ✅
- **文件**: `src/agents/orchestrator.py`
- **修复**: 添加"分析"等文档相关关键词到意图检测，避免 PDF 分析请求被误路由

### BF-5. system_prompt 硬编码示例 ✅
- **文件**: `src/agents/tutor.py`
- **修复**: 移除 system_prompt 中硬编码的"Python 异步编程"示例，避免 LLM 在首次交互时照抄示例内容

### BF-6. 学习计划主题提取 ✅
- **文件**: `src/ui/renderer.py`
- **修复**: "生成学习计划"按钮优先使用 `_doc_meta.title`（如 PDF 标题），而非用户首条消息（如"分析"）

### BF-7. RAG 短查询扩展 ✅
- **文件**: `src/agents/orchestrator.py`
- **修复**: 当用户输入过短（如"分析"）且存在 doc_meta 时，自动扩展查询为文档标题+概述关键词

### BF-8. UI 3-Tab 重构 ✅
- **文件**: `src/ui/layout.py`, `src/ui/state.py`, `src/ui/styles.py`, `src/ui/renderer.py`
- **修复**: 将 6 阶段 Stepper（准备→规划→学习→测验→报告→完成）简化为 3 个可点击 Tab（规划/学习/测验），消除重复按钮问题

### BF-9. GitHub URL 识别 ✅
- **文件**: `src/agents/orchestrator.py`
- **修复**: 添加 GitHub URL 关键词检测，确保 GitHub 链接被正确路由到 RepoAnalyzer

### BF-10. 删除会话功能 ✅
- **文件**: `src/ui/state.py`
- **修复**: 新增 `delete_session()` 函数，支持从侧边栏删除历史会话
...
| **Day 9 →** | 端到端验收 + P3 打磨 + 演示视频 + 简历更新 | ⬜ 下一步 |
...
- [x] P2-3 LLM-based 意图识别 — ✅ 已实现（缓存+关键词+LLM分类+fallback）
- [x] P2-4 Tutor 流式输出 — ✅ 已实现（stream_response + 自动回退）
